...
Some CLDR features are complicated to implement, and get are omitted. In other cases developers create a library containing just the features they require. This leads to many libraries with partial functionality. Getting everything together in one library within the size constraints is challenging.

### Why "batteries included"? Can this library be modularized even further?

I feel that modularization should be informed by the actual dependencies that exist, and these were hard to know in advance. Now that the library has matured it may be possible to split things up into smaller packages.

However, some functions are interdependent and can't be easily separated from one another.  Calendar formatters need access to numbering systems, number formatters need arbitrary precision, etc. So it may not be possible to achieve a satisfactory level of modularity.
